...
name: hazon
description: |
  Custom skill for Hazon Bible App development.
...
# Hazon - Development Skill

## Project Overview

A Bible reading app built with React Native/Expo using React Native Elements UI framework. Features multiple translations, verse highlighting, tabs, and offline reading.
...
- **Routing**: Expo Router
- **Storage**: expo-file-system (mobile) / IndexedDB (web) for Bible translations, AsyncStorage (for settings)
...
- Download translations as XML, parse and store in expo-file-system (mobile) or IndexedDB (web)
- Translation names formatted with acronyms (NIV, ESV, KJV)

### Storage
- expo-file-system: Downloaded Bible XML data on mobile (no size limits)
- IndexedDB: Downloaded Bible XML data on web (no size limits)
...
3. Download → stored in expo-file-system (mobile) or IndexedDB (web)
...
### UI Changes
1. App uses React Native Elements components
...
src/
  services/
    bible.ts      # Bible API, storage, downloads
  store/
